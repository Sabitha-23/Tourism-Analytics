...
def load_models():
    ...
    att_sim      = joblib.load("models/attraction_similarity.pkl")
    scaler       = joblib.load("models/rec_scaler.pkl")
...
# ── RECOMMENDATIONS ────────────────────────────────────────────────────────────
    ...
    user_id = st.number_input("Enter User ID", min_value=1, value=70456)
    ...
    def collaborative_filtering(user_id, n_recommendations=5):
        ...
        if not recommendations:
            top = df.groupby('AttractionId')['Rating'].mean()\
        ...
        att_names = df[['AttractionId','Attraction','AttractionType']]\
            .drop_duplicates('AttractionId')
        return rec_df.merge(att_names, on='AttractionId')[['Attraction','AttractionType','Rating']]
    ...
    def content_based_filtering(user_id, n_recommendations=5):
        ...
        att_names = df[['AttractionId','Attraction','AttractionType','Attraction_Avg_Rating']]\
            .drop_duplicates('AttractionId')
